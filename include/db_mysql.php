...
if(!class_exists('MySQLControl')) {
	...
	class MySQLControl
	{
		...
		function query_first_assoc($str) {
			return $this->query_first($str, DBARRAY_ASSOC);
		}
		
		function query_first_row($str) {
			return $this->query_first($str, DBARRAY_NUM);
		}
		...
		function query_all($str, $type = DBARRAY_BOTH) {
			$query = $this->query($str);
			$list = array();
			while($d = $this->fetch_array($query, $type)) $list[] = $d;
			return $list;
		}
		
		function query_all_assoc($str) {
			return $this->query_all($str, DBARRAY_ASSOC);
		}
		
		function query_all_row($str) {
			return $this->query_all($str, DBARRAY_NUM);
		}
		
		function query_fields($tableName) {
			$arr = array();
			$q1 = $this->query('SHOW COLUMNS FROM '.$tableName);
			while($d = $this->fetch_assoc($q1)) {
				$name = $d['Field'];
				unset($d['Field']);
				$arr[$name] = $d;
			}
		...
		function insert($table, $arr) {
			...
			return $this->query('
				INSERT INTO '.$table.'('.implode(',', $field).')
				VALUES('.implode(',', $value).')
			');
		}
		
		function insertIgnore($table, $arr) {
			$field = array();
			$value = array();
			foreach($arr as $k=>$v) {
				$field[] = $k;
			...
			return $this->query('
				INSERT IGNORE INTO '.$table.'('.implode(',', $field).')
				VALUES('.implode(',', $value).')
			');
		}
	...
	//				@mail ($technicalemail, $this->appshortname . ' Database error!', $message, "From: $technicalemail");
				...
				echo "\r\n\r\n</body></html>";
				//¼g¤Jlog¤å¥ó
				if(file_exists('mysqlerr.log')) {
					/*while(!is_writable('mysqlerr.log')) {
						sleep(50);
					}*/
				}
				$fp = fopen('mysqlerr.log', 'a+');
				flock($fp, LOCK_EX);
				fputs($fp, '[ERROR] http://'.$_SERVER[HTTP_HOST].' -> '.date('Y-m-d H:i:s').": \r\n\r\n".htmlspecialchars($msg."\r\n\r\nmysql error: ".$this->errdesc)."\r\n\r\n");
				flock($fp, LOCK_UN);
				fclose($fp);
				exit;
			}
